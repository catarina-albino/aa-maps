...
public class PontosNegrosPortugalGrid {
	...
	public static void createTable(String name) {
		...
		accPortugal.add(new Column("x", false, false, "NUMERIC"));
		accPortugal.add(new Column("y", false, false, "NUMERIC"));
	...
	public static void insertGridData(String fileName) throws Exception {
		...
		try {
			...
			while( (line = mapReader.read(header, processors)) != null ) {
				...
				ps.setInt(9, Integer.parseInt(x));
				ps.setInt(10, Integer.parseInt(y));
				ps.addBatch();
	...
	public static void main(String[] args) {
		final File folder = new File("data/pontosNegros");
		getDatasetFiles(folder);

		for (int i=0; i< files.size(); i++){
			String file = files.get(i);
			PontosNegrosPortugalGrid.createTable("pontos_negros_janela_fixa"+file.substring(file.lastIndexOf('_')+1, file.lastIndexOf('.')));
			try {
				PontosNegrosPortugalGrid.insertGridData("data/pontosNegros/"+file);
			} catch (Exception e) {
				e.printStackTrace();
			}
	    }
